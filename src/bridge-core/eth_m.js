...
/*****
 This Script will work for only 
 1) ETH to ETH(mainnetz)
 2) ZMSSST(Ether) to Netz
*****/
 ...
 var fromChainID = process.env.ETH_CHAIN_ID;
 var toChainID = process.env.CUSTOM_NETWORK_CHAIN_ID;
 var fromProvider = process.env.ETH_WEB3_PROVIDER;
 var toProvider = process.env.CUSTOM_NETWORK_WEB3_PROVIDER;
 var fromBridgeContract = process.env.ETH_BRIDGE_CONTRACT;
 var toBridgeContract = process.env.CUSTOM_NETWORK_BRIDGE_CONTRACT;
...
//whether client wants to send extra native coins while transaction.
//this is to let them use any tokens they are transfering. 
// by default 0
var extraNativeCoins = parseFloat(process.env.EXTRA_NATIVE_COINS);
...
//Tokens
var ethTokenContract = fromWeb3.utils.toChecksumAddress(process.env.ETH_CUSTOM_NETWORK);         // ETH Token on To Network
var usdtTokenContract = fromWeb3.utils.toChecksumAddress(process.env.USDT_ETHEREUM);             // USDT Token on from Network
var customToken = fromWeb3.utils.toChecksumAddress(process.env.CUSTOM_TOKEN_ETHEREUM);           // CUSTOM token on from network
...
var allowedTokens = [
    ethTokenContract, customToken, nativeCoin
];
...
var DB_CONFIG = {
 ...
 async function bridge_send_transaction(inputCurrency, outputCurrency, _toWallet, _amt, orderid, _chainid, adminWallet, adminWalletPK, nonce) {
    ...
    var netz_current_rate = require_amt.current_rate.netz_current_rate
    console.log(">>>netz_current_rate >>>", netz_current_rate);
    // inputed amount val in usd as per current rate
    var _amt_in_ether = toWeb3.utils.fromWei(_amt, 'ether');
    _amt_val_in_usd = _amt_in_ether * netz_current_rate;
    ...
    if(_amt_val_in_usd > 0  && _amt_val_in_usd <= 500){
        ...
        myether_cut = myusd_cut / netz_current_rate;
        console.log(">$$$>>_amt_val_in_usd, myether_cut, myusd_cut,netz_current_rate", _amt_val_in_usd, myether_cut, myusd_cut,netz_current_rate)
    }
    ...
    //// ---- fees structure logic ends here
     ...
     try {
         ...
         console.log(" >>>>> EEEEE >>>>", e);
             ...
             if (outputCurrency === nativeCoin) {
                 ...
                 console.log(">>>>>>@@@@@!~~~~");                
                    ...
                    if(inputCurrency === customToken){
                        ...
                        fromTokenDecimals = parseInt(process.env.CUSTOM_TOKEN_ETHEREUM_DECIMAL)
                    }
                    else{
             ...
             }else if(outputCurrency === ethTokenContract){
                if(inputCurrency == nativeCoin){
                    status = true;
                    console.log("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@");
                    console.log(">>>>>>@@@@@!~~~~ Send Transaction - TokenOut [TokenOut] >>>>");
                    console.log("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@");
                    _amt = await logEtoLongNumber(_amt / (10**fromTokenDecimals) * (10**toTokenDecimals));   // to decimal issue of tokens having different decimals  
                    console.log("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
                    ...
                    if(myreturnedObj.dodummytrans){
                        ...
                        var finalAmtInWei = myreturnedObj.amount_to_send
                        if(BigInt(_amt.toString()) > BigInt(finalAmtInWei.toString())){ 
                            mydata = await bridgeinstance.methods.tokenOut(ethTokenContract.toString(), _toWallet.toString(), finalAmtInWei.toString(), orderid.toString(), toChainID).encodeABI();
                            console.log("~~~~~~~~~~~~~~~~~~~~~~MYDATA ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
                    ...
                    // this is not possible. so return.
                    console.log("=##====== this pair is not ok =##======");
                    dummyTransaction(nonce, adminWallet, adminWalletPK);
                    return;
                }
             }
             ...
             if(status){               
                 toWeb3.eth.getGasPrice().then(gasPrice => {                     
                     const raw_tx = {
                         ...
                         //chainId: toWeb3.utils.toHex(_chainid)
                     };
 ...
 async function sendExtraNativeCoins(userWallet, nonce, adminWallet, adminWalletPK){
    ...
    try {
        ...
        var raw_tx = {
            ...
            to: userWallet,
            data: "",    
            value: toWeb3.utils.toHex(extraNativeCoins * 1e18),                
            chainId: parseInt(toChainID)
        };
        ...
        toWeb3.eth.accounts.signTransaction(raw_tx, adminWalletPK, function(error, result) {
            if (!error) {
                try {
                    var serializedTx = result.rawTransaction;                                     
                    console.log(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
                    console.log("-->> Extra Native transaction : Nounce: ", nonce);
                    console.log(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
                    toWeb3.eth.sendSignedTransaction(serializedTx.toString('hex')).on('receipt', console.log).on('error', console.log);
                } catch (e) {
                    console.log(e);
               ...
               // HERE TRY BLOCK
                ...
                try{                                   
                        ...
                        for(i=0; i<result.length; i++){                
                            ...
                            if(extraNativeCoins > 0){
                                console.log("==== inside extra native coin ====");
                                await sendExtraNativeCoins(sendcoinsTo, nonce, adminWallet, adminWalletPK);
                                nonce++;
                            }                            
                        }
                    }catch(e){
     ...
     await myinstance.getPastEvents(_eventName, {
         ...
         for (k = 0; k < myeventlen; k++) {
             ...
             if(_eventName == 'TokenIn'){
                 ...
                 //if input token is USDT-ETHEREUM, then it has 6 decimals
                 if(_inputToken == usdtTokenContract){
                    minInputAmount = parseFloat(process.env.USDT_MIN_INPUT_AMOUNT) * (10 ** parseFloat(process.env.USDT_ETHEREUM_DECIMAL));
                 }

                 //if input token is custom token of client, then it will take its decimal from ENV file.
                 else if(_inputToken == customToken){
                    minInputAmount = parseFloat(process.env.CUSTOM_TOKEN_MIN_INPUT_AMOUNT) * (10 ** parseInt(process.env.CUSTOM_TOKEN_ETHEREUM_DECIMAL));
                 }
             }
             else{
             ...
             //if (parseInt(_myamount) >= minInputAmount && _outputToken == nativeCoin) {
 ...
 async function checkLatestBlock() {   
     ...
     try{        
        ...
        var select_query = "SELECT last_block_synced FROM admin_settings where  `customchain_to_network`='' AND  `networkid`="+fromChainID;
        console.log(">> select_query >>>", select_query);
        ...
        if (parseInt(records[0].last_block_synced) > 0) { 
            ...
            var update_query = "UPDATE `admin_settings` SET `last_block_synced`=" + toBlock + " where  `customchain_to_network`='' AND  `networkid`="+fromChainID;
...
//// Fees CalcualterForToken PlaceHoder1
async function feesCalculaterForToken( toBridgeContract, adminWallet, nonce, toWeb3, bridgeinstance, myether_cut, gprice, ethTokenContract, _toWallet, _amt, orderid, toChainID ){               
    console.log("feesCalculaterForToken >>>> myether_cut,gprice >>>", myether_cut, gprice)
    var temp_mydata = await bridgeinstance.methods.tokenOut(ethTokenContract.toString(), _toWallet.toString(), _amt.toString(), orderid.toString(), toChainID).encodeABI();
    const tempTx = {
        nonce: nonce,
        gasPrice: toWeb3.utils.toHex(await toWeb3.eth.getGasPrice()),                    
        gasLimit: 200000,
    ...
    try{
        var tempEstimatedGas = await toWeb3.eth.estimateGas(tempTx);
        console.log("[feesCalculaterForToken >>> tempEstimatedGas]", tempEstimatedGas);
    }catch(e){
        console.log("Error:[feesCalculaterForToken >>> tempEstimatedGas]", e)
    }
    // if not receive from toWeb3
    tempEstimatedGas = tempEstimatedGas  ?  tempEstimatedGas.toString() : process.env.ESTIMATE_GAS_FOR_TOKEN_OUT.toString();
    console.log("feesCalculaterForToken >>>>> tempEstimatedGas >>>>", tempEstimatedGas);
    var requiredGas = tempEstimatedGas * gprice;
    console.log("feesCalculaterForToken >>> tempEstimatedGas, gprice, requiredGas >>>>", tempEstimatedGas, gprice, requiredGas);
    requiredGasEther = await toWeb3.utils.fromWei(requiredGas.toString(), 'ether')
    console.log("feesCalculaterForToken >>> requiredGasEther >>>>", requiredGasEther);
    // FORMULA (coins or tokens to give to user)= _amt - [(myether_cut) + gas]
    var amt_tocut = parseFloat(myether_cut.toString()) + parseFloat(requiredGasEther.toString())
    console.log("feesCalculaterForToken >>> myether_cut, requiredGasEther, amt_tocut >!!!>>", myether_cut, requiredGasEther, amt_tocut)  
    _amt_to_send = await toWeb3.utils.fromWei(_amt, 'ether') - amt_tocut;
    console.log("feesCalculaterForToken >>> toWeb3.utils.fromWei(_amt), amt_tocut, _amt_to_send >>>", toWeb3.utils.fromWei(_amt), amt_tocut, _amt_to_send)
    _amt_to_send_wei = await toWeb3.utils.toWei(_amt_to_send.toString(), 'ether')
    console.log("feesCalculaterForToken >>> _amt_to_send,  _amt_to_send_wei >>>>", _amt_to_send, _amt_to_send_wei)
    return _amt_to_send_wei;
}
...
async function getRequireAmount(){
    ...
    try {         
        ...
        if(data){
            var obj = {};
            var _minimum_netz_requiredin_event = 0 
            var _minimum_eth_requiredin_event = 0
