...
*/
 ...
 var fromChainID = process.env.POLYGON_CHAIN_ID;
 var toChainID = process.env.CUSTOM_NETWORK_CHAIN_ID;
 var fromProvider = process.env.POLYGON_WEB3_PROVIDER;
 var toProvider = process.env.CUSTOM_NETWORK_WEB3_PROVIDER;
 var fromBridgeContract = process.env.POLYGON_BRIDGE_CONTRACT;
...
var maticTokenContract = fromWeb3.utils.toChecksumAddress(process.env.MATIC_CUSTOM_NETWORK);       // Matic Token on To Network


// Add here both input and output tokens
var allowedTokens = [
    nativeCoin, maticTokenContract
];
...
async function bridge_send_transaction(inputCurrency, outputCurrency, _toWallet, _amt, orderid, _chainid, adminWallet, adminWalletPK, nonce) {
    ...
    //console.log(">>> require_amt.current_rate.matic_current_rate >>>", require_amt.current_rate.matic_current_rate);
    var matic_current_rate = require_amt.current_rate.matic_current_rate
    console.log(">>>matic_current_rate >>>", matic_current_rate);
    // inputed amount val in usd as per current rate
    var _amt_in_ether = toWeb3.utils.fromWei(_amt, 'ether');
    _amt_val_in_usd = _amt_in_ether * matic_current_rate;
    ...
    if(_amt_val_in_usd > 0  && _amt_val_in_usd <= 500){
        ...
        myether_cut = myusd_cut / matic_current_rate;
        console.log("#>Condition1>>_amt_val_in_usd, myether_cut, myusd_cut, matic_current_rate", _amt_val_in_usd, myether_cut, myusd_cut, matic_current_rate)
    }
     ...
     try {
         ...
         console.log(" >>>>> EEEEE >>>>", e);
             ...
             if (outputCurrency === nativeCoin) {
                ...
                //this is not needed now. so, return
                console.log("====== no native coins were sent =======");
                dummyTransaction(nonce, adminWallet, adminWalletPK);
                return;
                 ...
                 console.log(">>>>>>@@@@@!~~~~");                
                    ...
                    if(inputCurrency === nativeCoin){
                        _amt = parseFloat(pricedata[3].tokenPriceInUSD) * _amt / parseFloat(pricedata[4].tokenPriceInUSD); 
                        console.log(">>>>> pricedata[3].tokenPriceInUSD >>>>", pricedata[3].tokenPriceInUSD); 
                    }
                    else{
             ...
             }else if(outputCurrency === maticTokenContract){
                if(inputCurrency == nativeCoin){
                    status = true;
                    console.log("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@");
                    ...
                    if(myreturnedObj.dodummytrans){
                        ...
                        if(BigInt(_amt.toString()) > BigInt(finalAmtInWei.toString())){                     
                            mydata = await bridgeinstance.methods.tokenOut(maticTokenContract.toString(), _toWallet.toString(), finalAmtInWei.toString(), orderid.toString(), toChainID).encodeABI();
                            console.log("~~~~~~~~~~~~~~~~~~~~~~MYDATA ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
 ...
 async function getTransactionsFromDB() {     
     try {
        var adminWallet = process.env.ADMIN_WALLET_POLYGON;
        var adminWalletPK = process.env.ADMIN_WALLET_POLYGON_PK;
 ...
 async function getEventData(_fromBlock, _toBlock, _eventName) {
     ...
     await myinstance.getPastEvents(_eventName, {
         ...
         for (k = 0; k < myeventlen; k++) {
             ...
             // Also it will prevent any output coins from being a native coin          
 ...
 async function checkLatestBlock() {   
     ...
     //token in event is not needed. Please activate if that pair is added
...
async function getRequireAmount(){
    ...
    try {         
        var query = util.promisify(Connection.query).bind(Connection);         
        var sel_query = "SELECT `tokenName`, `tokenPriceInUSD` FROM token_pricing_table where tokenName IN ('Matic', 'clientChainNativeCoin')";
        var data = await query(sel_query).catch(console.log);
        console.log(">>>> data >>>",  data);
        
        if(data){
            var obj = {};
            var matic_current_rate = 0;
            var netz_current_rate = 0;
            data.forEach((myelement, index)=>{               
                if(myelement.tokenName === 'Matic'){
                    if(myelement.tokenPriceInUSD > 0){
                        matic_current_rate = myelement.tokenPriceInUSD;
                    }
            ...
            var current_rate = {
                'matic_current_rate': matic_current_rate,
