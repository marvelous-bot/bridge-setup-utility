...
*/
 ...
 var fromChainID = process.env.BSC_CHAIN_ID;
 var toChainID = process.env.CUSTOM_NETWORK_CHAIN_ID;
 var fromProvider = process.env.BSC_WEB3_PROVIDER;
 var toProvider = process.env.CUSTOM_NETWORK_WEB3_PROVIDER;
 var fromBridgeContract = process.env.BSC_BRIDGE_CONTRACT;
...
//Tokens
var bnbTokenContract = fromWeb3.utils.toChecksumAddress(process.env.BNB_CUSTOM_NETWORK);         // BNB Token on To Network
var usdtTokenContract = fromWeb3.utils.toChecksumAddress(process.env.USDT_BSC);                  // USDT Token on from Network
...
var allowedTokens = [
 ...
 async function bridge_send_transaction(inputCurrency, outputCurrency, _toWallet, _amt, orderid, _chainid, adminWallet, adminWalletPK, nonce) {
    ...
    //console.log(">>> require_amt.current_rate.bnb_current_rate >>>", require_amt.current_rate.bnb_current_rate);
    ...
    //// ---- fees structure logic ends here
     ...
     try {
         ...
         console.log(" >>>>> EEEEE >>>>", e);
             ...
             if(outputCurrency === bnbTokenContract){
                if(inputCurrency == nativeCoin){
                    status = true;
                    console.log("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@");
                    ...
                    if(myreturnedObj.dodummytrans){
                        ...
                        if(BigInt(_amt.toString()) > BigInt(finalAmtInWei.toString())){   
                            mydata = await bridgeinstance.methods.tokenOut(bnbTokenContract.toString(), _toWallet.toString(), finalAmtInWei.toString(), orderid.toString(), toChainID).encodeABI();
                            console.log("~~~~~~~~~~~~~~~~~~~~~~MYDATA ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
     ...
     await myinstance.getPastEvents(_eventName, {
         ...
         for (k = 0; k < myeventlen; k++) {
             ...
             if(_eventName == 'TokenIn'){
                 ...
                 //if input token is USDT-BSC, then it has 18 decimals
                 if(_inputToken == usdtTokenContract){
                    minInputAmount = parseFloat(process.env.USDT_MIN_INPUT_AMOUNT) * (10 ** 18);
                 }

                 //if input token is custom token of client, then it will take its decimal from ENV file.
                 else if(_inputToken == customToken){
                    minInputAmount = parseFloat(process.env.CUSTOM_TOKEN_MIN_INPUT_AMOUNT) * (10 ** parseInt(process.env.CUSTOM_TOKEN_BSC_DECIMAL));
                 }
             }
             else{
                // This condition if for BNB only, and it only has 18 decimals
                minInputAmount = parseFloat(process.env.BNB_MIN_INPUT_AMOUNT) * (10 ** 18);
             }
