...
/// ToChain is Polygon-Matic
/// pair - MATIC(mainnetz)  to MATIC (Polygon)
/////////////////////////////////////////////////////////////////////
 ...
 var toChainID = process.env.POLYGON_CHAIN_ID;

 var fromProvider = process.env.CUSTOM_NETWORK_WEB3_PROVIDER;
 var toProvider = process.env.POLYGON_WEB3_PROVIDER;

 var fromBridgeContract = process.env.CUSTOM_NETWORK_BRIDGE_CONTRACT;
 var toBridgeContract = process.env.POLYGON_BRIDGE_CONTRACT;
...
//Tokens
var customToken = fromWeb3.utils.toChecksumAddress(process.env.MATIC_CUSTOM_NETWORK);           // MATIC token on from network
             ...
             if (outputCurrency === nativeCoin) {
                 ...
                 console.log(">>>>>>@@@@@!~~~~");                
                    ...
                    if(inputCurrency === customToken){
                        ...
                        fromTokenDecimals = parseInt(process.env.MATIC_CUSTOM_NETWORK_DECIMAL)
                    }
                    else{
...
async function dummyTransaction(nonce, adminWallet, adminWalletPK){
         ...
         for (k = 0; k < myeventlen; k++) {
             ...
             if(_eventName == 'TokenIn'){
                 ...
                 if(_inputToken == customToken){
                    minInputAmount = parseFloat(process.env.CUSTOM_TOKEN_MIN_INPUT_AMOUNT) * (10 ** parseInt(process.env.MATIC_CUSTOM_NETWORK_DECIMAL));
                 }
             }
             else{
 ...
 async function checkLatestBlock() {   
     ...
     try{        
        ...
        var select_query = "SELECT last_block_synced FROM admin_settings where customchain_to_network='matic' AND `networkid`="+fromChainID;
        ...
        if (parseInt(records[0].last_block_synced) > 0) { 
            ...
            var update_query = "UPDATE `admin_settings` SET `last_block_synced`=" + toBlock + " where customchain_to_network='matic' AND `networkid`="+fromChainID;
