 ...
 * This bridge supports 2 chain IDs to covert from and to any network.
...
/// ToChain is Ethereum-ETH
/// pair - peggy ETH(mainnetz)  to ETH (ethereum)
/////////////////////////////////////////////////////////////////////
 ...
 var toChainID = process.env.ETH_CHAIN_ID;

 var fromProvider = process.env.CUSTOM_NETWORK_WEB3_PROVIDER;
 var toProvider = process.env.ETH_WEB3_PROVIDER;

 var fromBridgeContract = process.env.CUSTOM_NETWORK_BRIDGE_CONTRACT;
 var toBridgeContract = process.env.ETH_BRIDGE_CONTRACT;

 var fromTokenDecimals = 18;
 var toTokenDecimals = 18;

 var maxTxnsInBatchETH = process.env.MAX_TRANSACTION_IN_BATCH_ETH; 
 ...
 var CONTRACT_ADDR_ABI = JSON.parse(process.env.BATCH_BRIDGE_ABI);
 

//Tokens
var customToken = fromWeb3.utils.toChecksumAddress(process.env.ETH_CUSTOM_NETWORK);           // ETH token on from network
...
async function bridge_send_transaction(inputCurrency, outputCurrency, _toWallet, _amt, orderid, _chainid, adminWallet, adminWalletPK, nonce) {
    ...
    var eth_current_rate = require_amt.current_rate.eth_current_rate
    console.log(">>>eth_current_rate >>>", eth_current_rate);
    // inputed amount val in usd as per current rate
    var _amt_in_ether = toWeb3.utils.fromWei(_amt, 'ether');
    _amt_val_in_usd = _amt_in_ether * eth_current_rate;
    ...
    if(_amt_val_in_usd > 0  && _amt_val_in_usd <= 500){
        ...
        myether_cut = myusd_cut / eth_current_rate;
        console.log("#>Condition1>>_amt_val_in_usd, myether_cut, myusd_cut, eth_current_rate", _amt_val_in_usd, myether_cut, myusd_cut, eth_current_rate)
    }
    ...
    //// ---- fees structure logic ends here
     ...
     console.log("~~~~~~~~~~~~~~~~~@@@@@@@@@@~~~~~~~~~~~~~~~~~~~~~~~~~");
     console.log(">>>> in bridge_send_transaction function >>>>");    
     ...
     try {
         ...
         console.log(" >>>>> EEEEE >>>>", e);
             ...
             if (outputCurrency === nativeCoin) {
                 ...
                 console.log(">>>>>>@@@@@!~~~~");                
                    ...
                    if(inputCurrency === customToken){
                        ...
                        fromTokenDecimals = parseInt(process.env.ETH_CUSTOM_NETWORK_DECIMAL)
                    }
                    else{
                    ...
                    if(myreturnedObj.dodummytrans){
                        ...
                        console.log(">> incrementing batchdummyCounter >>>", batchdummyCounter)
                        batchdummyCounter++;
                        return;
                    }else{
                        ...
                        if(BigInt(_amt.toString()) > BigInt(finalAmtInWei.toString())){  
                            ...
                            var tobj = {};
                            tobj =  {"orderid":orderid.toString(), "_toWallet":_toWallet.toString(), "finalAmtInWei": finalAmtInWei.toString()};
                            batchArray.push(tobj);
                            console.log(">>>> CoinOut >> batchArray >>>>", batchArray);
                        }else{
            ...
            console.log("\n\n##################################")      
            console.log(">> batchdummyCounter, batchArray.length, process.env.TOTAL_COIN_OUT_RECORDS_FROM_DB >>", batchdummyCounter, batchArray.length, process.env.TOTAL_COIN_OUT_RECORDS_FROM_DB)

            var totalRecords = parseInt(batchdummyCounter) + parseInt(batchArray.length);
           
            if(totalRecords >= process.env.TOTAL_COIN_OUT_RECORDS_FROM_DB) {
                batchdummyCounter=0;
                // set new batchnumber for next batch
                var tempBatchArray = []
                console.log("\n\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
                console.log(">>> batchArray.length >>>", batchArray.length);
                console.log("\n\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
                // lets move first 20 rocords in array
                //tempBatchArray.push(... batchArray.splice(0,20));
                tempBatchArray.push(... batchArray.splice(0,batchArray.length));
                console.log("\n\n~~~~ tempBatchArray.length >>>", tempBatchArray.length);
                batchnumber = batchnumber + 1;
                console.log(`>>> sending batchnumber -${batchnumber}, sending data, tempBatchArray >> `,tempBatchArray)
                console.log("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
                var tempOrderidsary = [];
                var tempTowalletary = [];
                var tempFinalamtinwei = [];
                tempBatchArray.forEach((obj)=>{ 
                    console.log(obj.orderid, obj._toWallet, obj.finalAmtInWei) 
                    tempOrderidsary.push(obj.orderid);
                    tempTowalletary.push(obj._toWallet);
                    tempFinalamtinwei.push(obj.finalAmtInWei);
                })
                console.log(">>> tempOrderidsary, tempTowalletary, tempFinalamtinwei >>>", tempOrderidsary, tempTowalletary, tempFinalamtinwei)
                //console.log(">>> tempBatchArray >>>>", tempBatchArray);
         
                console.log(`\n>>>~~~~ \n New Batch received..,  tempBatchArray.length-${tempBatchArray.length}, tempBatchArray-`, tempBatchArray); 
                // create mydata here
                var _mydata = await bridgeinstance.methods.batchCoinOut(tempTowalletary, tempFinalamtinwei, tempOrderidsary).encodeABI();

                console.log(mydata);
                var _mynonce = await toWeb3.eth.getTransactionCount(adminWallet, "pending");
                console.log(">>> _mynonce >>>",  _mynonce)
                /// this is checked in cron 
                 process.env.LASTETHSENTTIMESTAMP = Date.now();
...
async function dummyTransaction(nonce, adminWallet, adminWalletPK){
            ...
            console.log("<<@@@>@@@>>No admin wallet bridge available ...<@@@@@@>>");                         
               ...
               // HERE TRY BLOCK
                ...
                try{                                   
                        ...
                        var query2 = "select * from bridge_transactions where status=0 and fromChainID=" + fromChainID + " and toChainID=" + toChainID + " limit "+maxTxnsInBatchETH;

                        console.log(">>selecting from DB...., select QUERY<<", query2)
                        var result = await util_bridgequery(query2);
                        /// here
                        process.env.TOTAL_COIN_OUT_RECORDS_FROM_DB = result.length;
         ...
         for (k = 0; k < myeventlen; k++) {
             ...
             if(_eventName == 'TokenIn'){
                 ...
                 if(_inputToken == customToken){
                    minInputAmount = parseFloat(process.env.CUSTOM_TOKEN_MIN_INPUT_AMOUNT) * (10 ** parseInt(process.env.ETH_CUSTOM_NETWORK_DECIMAL));
                 }
             }
             else{
 ...
 async function checkLatestBlock() {   
     ...
     try{        
        ...
        var select_query = "SELECT last_block_synced FROM admin_settings where customchain_to_network='eth' AND `networkid`="+fromChainID;
        ...
        if (parseInt(records[0].last_block_synced) > 0) { 
            ...
            var update_query = "UPDATE `admin_settings` SET `last_block_synced`=" + toBlock + " where customchain_to_network='eth' AND `networkid`="+fromChainID;
 ...
 async function feesCalculaterForCoins( toBridgeContract, adminWallet, nonce, toWeb3, bridgeinstance, myether_cut, gprice, _toWallet, _amt, orderid, toChainID ){               
    ...
    // from here network fees
    //add additional network fees for ethereum network ----
    //network fees = Gas price + estimated Gas Usage , on this take a cut
    var fixedGPriceInEther = '0.000000025'; // considering 25gwei 
    var gPriceInEther = (await toWeb3.utils.fromWei(gprice.toString(), 'ether') > 0) ? (await toWeb3.utils.fromWei(gprice.toString(), 'ether')) : fixedGPriceInEther;
    var networkfees = parseFloat(gPriceInEther.toString()) + parseFloat(requiredGasEther.toString());
    var network_fees = networkfees.toFixed(18);
    console.log("\n\n#######################################################################")
    console.log(">>> gprice, gPriceInEther, requiredGasEther, networkfees, network_fees", gprice, gPriceInEther, requiredGasEther, networkfees, network_fees.toString());
    console.log("\n\n#######################################################################")

    /// upto here network fees
    // FORMULA (coins or tokens to give to user)= _amt - [(myether_cut) + gas]
    var amt_tocut = parseFloat(myether_cut.toString()) + parseFloat(requiredGasEther.toString()) + parseFloat(networkfees.toString())
 ...
 async function getRequireAmount(){
    ...
    try {         
        var query = util.promisify(Connection.query).bind(Connection);         
        var sel_query = "SELECT `tokenName`, `tokenPriceInUSD` FROM token_pricing_table where tokenName IN ('ETH', 'clientChainNativeCoin')";
        var data = await query(sel_query).catch(console.log);
        console.log(">>>> data >>>",  data);
        
        if(data){
            var obj = {};
            var eth_current_rate = 0;
            var netz_current_rate = 0;
            data.forEach((myelement, index)=>{               
                if(myelement.tokenName === 'ETH'){
                    if(myelement.tokenPriceInUSD > 0){
                        eth_current_rate = myelement.tokenPriceInUSD;
                    }
            ...
            var current_rate = {
                'eth_current_rate': eth_current_rate,
...
//Cron running Every min
var job = new CronJob('0 */2 * * * *', function() {
    console.log("\n\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
    console.log('Cron running, every min');
    console.log(">>> batchArray.length, batchdummyCounter >>>", batchArray.length, batchdummyCounter)
    console.log("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n\n");
    process.env.LASTETHSENTTIMESTAMP = ( process.env.LASTETHSENTTIMESTAMP > 0)  ?  process.env.LASTETHSENTTIMESTAMP : 0;
    console.log(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
    console.log(">>>> process.env.LASTETHSENTTIMESTAMP >>>>", process.env.LASTETHSENTTIMESTAMP);
    console.log("-------------------------------------");
    
    checkLatestBlock();
    // check for last sent timestamp .. ethereum taking time to get transaction confirm/ complete
    // 2 mins timestamp 
    var timespan = 2 * 60 * 1000;
    var tmstamp = Number(process.env.LASTETHSENTTIMESTAMP) + Number(timespan);
    var currenttimestamp = Date.now();
    /// if currenttimestamp > (lasttimestamp + 2) mins then can try other transactions so less revert of failure issues. 
    if( Number(currenttimestamp) > Number(tmstamp) ){
        console.log(">>> timestamp is greater than > 2 mins., lets pick transactions from db this time ..")
